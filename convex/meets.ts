...
function predictEventTime(
  ...
  if (!ESTIMATE_OK_EVENTS.has(targetEvent)) return null;
  ...
  for (const t of athleteTimes) {
    ...
    if (HURDLE_EVENTS.has(t.event) !== targetIsHurdle) continue;
...
function applyTeamLimit<T extends { teamSlug: string }>(rows: T[], limit: number): T[] {
...
// Shared by eventRankings query and meetProjectedScores query.
type RankingRow = { athleteId: string; athleteName: string; teamSlug: string; teamName: string; time: number; estimated: boolean };

async function computeEventTop8(
  ctx: any,
  event: string,
  ...
  let allMeetEntries: { teamSlug: string; event: string; athleteIds: string[] }[] = [];
  const submittedTeams = new Set<string>();
  const enteredAthletesForEvent = new Set<string>();
  if (meetId) {
    allMeetEntries = await ctx.db
      .query("meetEntries")
      .withIndex("by_meet", (q: any) => q.eq("meetId", meetId))
      .collect();
    for (const e of allMeetEntries) {
      submittedTeams.add(e.teamSlug);
      if (e.event === event) {
        for (const id of e.athleteIds) enteredAthletesForEvent.add(id);
      }
    }
  }
  ...
  const directTimes = await ctx.db
    .query("times")
    .withIndex("by_event", (q: any) => q.eq("event", event))
    .collect();
  directTimes.sort((a: any, b: any) => isField ? b.time - a.time : a.time - b.time);
  ...
  if (ESTIMATE_OK_EVENTS.has(event)) {
    ...
    const sourceEvents = Object.entries(EVENT_DISTANCES).filter(([name, dist]) => {
      ...
      if (HURDLE_EVENTS.has(name) !== targetIsHurdle) return false;
    ...
    for (const [srcEvent] of sourceEvents) {
      const srcTimes = await ctx.db
        .query("times")
        .withIndex("by_event", (q: any) => q.eq("event", srcEvent))
        .collect();
      srcTimes.sort((a: any, b: any) => a.time - b.time);
      for (const t of srcTimes.slice(0, 24)) {
        if (seenAthletes.has(t.athleteId)) continue;
        seenAthletes.add(t.athleteId);
        const athlete = await ctx.db.get(t.athleteId);
        if (!athlete || !athlete.team) continue;
        const allTimes = await ctx.db
          .query("times")
          .withIndex("by_athlete", (q: any) => q.eq("athleteId", t.athleteId))
          .collect();
        const pred = predictEventTime(
          allTimes.map((at: any) => ({ event: at.event, time: at.time })),
          event
        );
        if (!pred) continue;
        rows.push({
  ...
  if (meetId) {
    const overrides = await ctx.db
      ...
      .withIndex("by_meet", (q: any) => q.eq("meetId", meetId))
      .collect();
    for (const o of overrides.filter((o: any) => o.event === event && enteredAthletesForEvent.has(o.athleteId))) {
      if (seenAthletes.has(o.athleteId)) continue;
  ...
  handler: async (ctx, args) => {
    ...
    if (args.meetId) {
      ...
      const enteredTeams = new Set(
        entries.filter((e) => e.event === args.event).map((e) => e.teamSlug)
      );
      ...
      for (const row of rows) {
        if (submittedTeams.has(row.teamSlug) && !enteredTeams.has(row.teamSlug)) continue;
...
export const setRelayPrediction = mutation({
  ...
  handler: async (ctx, args) => {
    const existing = await ctx.db
      .query("meetEntries")
      .withIndex("by_meet_team", (q) =>
        q.eq("meetId", args.meetId).eq("teamSlug", args.teamSlug)
      )
...
export const projectedPoints = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const entry of entries) {
      if (RELAY_EVENTS.has(entry.event) || FIELD_EVENTS.has(entry.event)) continue;
      ...
      for (const athleteId of entry.athleteIds) {
        ...
        if (time == null) {
          const athleteAllTimes = await ctx.db
            ...
            .collect();
          const pred = predictEventTime(
            athleteAllTimes.map((t) => ({ event: t.event, time: t.time })),
            entry.event
          );
          if (pred) time = pred.time;
        }
      ...
      if (eventPoints > 0) result[entry.event] = eventPoints;
    }
...
export const meetProjectedScores = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const slug of meet.teamSlugs) teamPoints.set(slug, 0);

    // ---- Individual track events ----
    const allIndividualEvents = new Set(
      allEntries
    ...
    for (const event of allIndividualEvents) {
      ...
      for (const entry of teamEntries) {
        const ourAthletes: number[] = [];
        for (const athleteId of entry.athleteIds) {
          if (!athleteId) continue;
          const override = timeOverrides.find(
            (o) => o.event === event && o.athleteId === athleteId && o.teamSlug === entry.teamSlug
          );
          ...
          if (override) {
            ...
            if (t) {
              ...
              const athleteAllTimes = await ctx.db
                .query("times")
                .withIndex("by_athlete", (q) => q.eq("athleteId", athleteId as any))
                .collect();
              const pred = predictEventTime(
                athleteAllTimes.map((t) => ({ event: t.event, time: t.time })),
                event
          ...
          if (time != null) ourAthletes.push(time);
        }
    ...
    // ---- Relay events ----
    const allRelayEvents = new Set(
      allEntries.filter((e) => RELAY_EVENTS.has(e.event)).map((e) => e.event)
    );
    ...
    for (const event of allRelayEvents) {
      ...
      const enteredTeams = new Set(
        allEntries.filter((e) => e.event === event).map((e) => e.teamSlug)
      );

      const teamBestTime = new Map<string, number>();
      for (const r of allRelayTimes) {
        if (submittedTeams.has(r.team) && !enteredTeams.has(r.team)) continue;
        const cur = teamBestTime.get(r.team);
        if (cur == null || r.time < cur) teamBestTime.set(r.team, r.time);
      }
...
export const simulate = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const e of entries) {
      for (const id of e.athleteIds) athleteIdSet.add(id);
    }
    for (const m of fieldMarks) athleteIdSet.add(m.athleteId);
    ...
    const timesMap = new Map<string, { event: string; time: number }[]>();
    const athleteNames = new Map<string, string>();

    for (const id of athleteIdSet) {
      const athlete = await ctx.db.get(id as any);
      if (athlete && "name" in athlete) athleteNames.set(id, (athlete as any).name);
      const times = await ctx.db
        .query("times")
        .withIndex("by_athlete", (q) => q.eq("athleteId", id as any))
        .collect();
    ...
    const marksByEvent = new Map<string, typeof fieldMarks>();
    ...
    type PlaceRow = {
      ...
      athleteName: string;
      teamSlug: string;
      value: number; // seconds for track, meters for field
      estimated: boolean; // relay prediction used
      points: number;
    };
    ...
    const byEvent: EventResult[] = [];

    for (const event of allEventNames) {
      if (FIELD_EVENTS.has(event)) {
        // Field event — highest mark wins.
        // Merge explicit meetFieldMarks (actual meet marks) with meetEntries (PR marks).
        // meetFieldMarks take precedence per athlete; fall back to athlete's stored PR mark.
        type FieldRow = { athleteId: string; teamSlug: string; mark: number };
        const rowMap = new Map<string, FieldRow>(); // keyed by athleteId
        ...
        const places: PlaceRow[] = sorted.map((m, i) => {
        ...
        for (const entry of eventEntries) {
          ...
          for (let i = 0; i < legDistances.length; i++) {
            ...
            if (!athleteId) { valid = false; break; }
            ...
            const exactEventName = Object.entries(EVENT_DISTANCES).find(
            ...
            if (exact) {
              ...
              if (override) {
                ...
                if (targetDist > 200) { valid = false; break; }
                ...
                if (predictions.length === 0) { valid = false; break; }
                const totalWeight = predictions.reduce((s, p) => s + p.weight, 0);
                const weighted =
          ...
          if (valid) relayTimes.push({ teamSlug: entry.teamSlug, totalTime: total, estimated });
        }

        const sorted = relayTimes.sort((a, b) => a.totalTime - b.totalTime);
        const places: PlaceRow[] = sorted.map((r, i) => {
          const pts = PLACE_POINTS[i] ?? 0;
        ...
        const eventEntries = entriesByEvent.get(event) ?? [];
        type IndRow = { athleteId: string; teamSlug: string; time: number; estimated: boolean };
        const rows: IndRow[] = [];
          ...
          for (const athleteId of entry.athleteIds) {
            ...
            if (override) {
              ...
              if (t) {
                ...
                if (pred) rows.push({ athleteId, teamSlug: entry.teamSlug, time: pred.time, estimated: pred.estimated });
              }
            }
          }
        }
    ...
    byEvent.sort(
      (a, b) => (EVENT_ORDER.indexOf(a.event) ?? 99) - (EVENT_ORDER.indexOf(b.event) ?? 99)
    );
...
export const addTeam = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (meet.teamSlugs.includes(args.teamSlug)) return;
          ...
          const override = await ctx.db
            ...
            .filter((q) =>
              q.and(
                ...
                q.eq(q.field("athleteId"), oldId as any)
              )
            )
            .first();
